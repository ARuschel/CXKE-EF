...
from tools.tools import generate_timestamp, ensure_dir, save_model_info, save_training_log

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

@click.command()
@click.option('--model', default='Analogy', help='Embedding model (e.g. TransE)')
@click.option('--dataset', '-d', default='FB15K237', help='Dataset directory')
@click.option('--work_threads', '-wt', default=multiprocessing.cpu_count(), help='Work threads.')
@click.option('--train_times', '-t', default=10, help='Train times (or epochs).')
@click.option('--nbatches', '-n', default=100, help='Number of batches.')
@click.option('--alpha', '-a', default=0.1, help='Alpha.')
@click.option('--margin', '-m', default=1.0, help='Margin.')
@click.option('--bern', '-b', default=1, help='Bernoulli.')
@click.option('--dimension', '-dim', default=200, help='Dimension.')
@click.option('--ent_neg_rate', '-enr', default=4, help='Entity negative rate.')
@click.option('--rel_neg_rate', '-rnr', default=2, help='Relation negative rate.')
@click.option('--opt_method', '-opt', default='Adagrad', help='Optimizer (SGD, RMSprop, Adam, Adadelta or ADAGRAD).')
...
def main(model, dataset, work_threads, train_times, nbatches, alpha, margin, bern, dimension, \
 ent_neg_rate, rel_neg_rate, opt_method, test_link_prediction, test_triple_classification, model_info_file):
    ...
    sha = current_repo.head.object.hexsha

    # export_path = './models/{}/{}/'.format(dataset, timestamp)
    export_path = os.path.expanduser('~') + '/proj/XKE_results/' + dataset +'/' + model+'/'+timestamp
    ...
    # con.set_score_norm(model_info['score_norm'])


    #Models will be exported via tf.Saver() automatically.
    con.set_export_files(export_path + '/tf_model/' + "model.vec.tf", 0)
    ...
    #Train the model.
    logger.info('Start training {} model on {} dataset'.format(model_info['model'], model_info['dataset_name']))
    con.run()
